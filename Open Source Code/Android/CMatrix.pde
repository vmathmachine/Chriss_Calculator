public static class CMatrix { //Complex Matrix
  ...
  public String toString(int dig) { //cast to a string given a specified number of digits of precision
    ...
    String res = "[";        //initialize to opening left bracket
    for(int i=0;i<h;i++) {   //loop through all rows
      res+="[";              //start each row with left bracket
      for(int j=0;j<w;j++) { //loop through all columns
        if(elements[i][j].lazyabs()<threshold) { res+="0"; } //if this element is below our threshold, round down to 0
        else { res += elements[i][j].toString(dig); } //concatenate each individual element, outputted to the given amount of precision
        if(j!=w-1) { res+=","; }             //put a comma after all entries but the last
      }
      res+="]";                //end each row with a right bracket
      if(i!=h-1) { res+=","; } //put a comma after all rows but the last
    }
    return res+"]"; //close with right bracket, return result
  }
