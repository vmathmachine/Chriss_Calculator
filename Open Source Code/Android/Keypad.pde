...
public static class KeyPad implements Iterable<Button> { //a class for storing individual keypads
  ...
  static Action type(final Mmio io, final Button butt, final char... typers) { //this generates an action that types one from a set of characters
    if(typers.length==1) { //one character: just type one character
      return new Action() { public void act() { if(io.typer!=null) {
        io.typer.type(typers[0]); //types the character
      } } };
    }
    if(typers.length==2) { //two characters: alternate between typing those two characters
      return new Action() { public void act() { if(io.typer!=null) {
        ...
        else                       { io.typer.overtype(typers[1],io.typer.cursor-1); io.typer.adjust(); }
      } } };
    }
    return new Action() { public void act() { if(io.typer!=null) { //otherwise, we type whichever character corresponds to the number of times we've pressed the button
      ...
      else                                   { io.typer.overtype(typers[butt.pressCount%typers.length],io.typer.cursor-1); io.typer.adjust(); }
    } } };
  }
  
  static Action type(final Mmio io, final String typer) { //this just types a string right in front of the cursor
    return new Action() { public void act() { if(io.typer!=null) {
      io.typer.insert(typer); //inserts the string
    } } };
  }
  ...
  static Action act(final Mmio io, final int action) { //this performs a specified action
    switch(action) {
      ...
      //  if(io.ctrlHeld) { io.typer.ctrlDelete(); }
      //  else            { io.typer.delete(true,true,true); }
      //} } };
    }
    return null; //other
  }
  
  static Action typeAnsPrefix(final Mmio io, final Button butt, final char... typers) {
    if(typers.length==1) { //one character: just type one character
      return new Action() { public void act() { if(io.typer!=null) {
        if(io.typer==query && io.typer.size()==0) { io.typer.insert("Ans"+typers[0]); }
        else                                      { io.typer.type(        typers[0]); } //types the character
      } } };
    }
    if(typers.length==2) { //two characters: alternate between typing those two characters
      return new Action() { public void act() { if(io.typer!=null) {
        if(io.typer==query && io.typer.size()==0) { io.typer.insert("Ans"+typers[0]);           }
        else if((butt.pressCount&1)==0)           { io.typer.type(typers[0]);                   }
        else               { io.typer.overtype(typers[1],io.typer.cursor-1); io.typer.adjust(); }
      } } };
    }
    return new Action() { public void act() { if(io.typer!=null) { //otherwise, we type whichever character corresponds to the number of times we've pressed the button
  ...
  static Action typeDoubleAnsPrefix(final Mmio io, final Button butt, final char typer) {
    return new Action() { public void act() { if(io.typer!=null) {
      if(io.typer==query && io.typer.size()==1 && (butt.pressCount&1)==1) { io.typer.insert("Ans",0); io.typer.cursor+=3; }
      else { io.typer.type(typer); }
    } } };
  }
  
  static Action clear(final Mmio io, final Button butt) {
    return new Action() { public void act() { if(io.typer!=null) {
