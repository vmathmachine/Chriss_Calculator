public static class Textbox extends Panel {
  ...
  void eraseSelection() { //erases the highlighted selection
  ...
  void idlyUpdateCarets() { //updates the carets while you idly hold the cursor to the far left or far right
  ...
  static class CaretMover extends Box { //an invisible box whose hitbox allows us to move the textbox's caret
    ...
    void addCaretHandle() { //adds a single caret handle
      caretHandle = new TSHandle(buddy, buddy.caret, CENTER, false); //create a centered handle at the primary caret
    }
    
    boolean respondToChange(final Cursor curs, final byte code, boolean selected) { //responds to change in the cursor (0=release, 1=press, 2=move, 3=drag) (select iff cursor is already touching something)
    ...
    void processTextTouch(Cursor curs) { //this processes text being double tapped or pressed and held
  ...
  static class TSHandle extends Box { //text selection handle
    ...
    boolean respondToChange(final Cursor curs, final byte code, boolean selected) {
      boolean hitbox = hitbox(curs); //first, find if the cursor is inside the hitbox
